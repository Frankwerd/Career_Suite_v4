...
function getOrCreateSpreadsheetAndSheet() {
  ...
  if (FIXED_SPREADSHEET_ID && FIXED_SPREADSHEET_ID.trim() !== "" && FIXED_SPREADSHEET_ID !== "YOUR_SPREADSHEET_ID_HERE") {
    // Logger.log(`[SHEET_UTIL INFO] SPREADSHEET: Using Fixed ID: "${FIXED_SPREADSHEET_ID}"`);
    try {
      ss = SpreadsheetApp.openById(FIXED_SPREADSHEET_ID);
      // Logger.log(`[SHEET_UTIL INFO] SPREADSHEET: Opened "${ss.getName()}" (Fixed ID).`);
    } catch (e) {
    ...
    try {
      ...
      Logger.log(`[SHEET_UTIL FATAL] SPREADSHEET: DRIVE/OPEN FAIL - Error accessing Google Drive for "${TARGET_SPREADSHEET_FILENAME}". ${eDrive.message}.`);
      return { spreadsheet: null, sheet: null };
    }
  }
  ...
  if (ss) {
    appTrackerSheet = ss.getSheetByName(APP_TRACKER_SHEET_TAB_NAME); // APP_TRACKER_SHEET_TAB_NAME from Config.gs
    if (!appTrackerSheet) {
      Logger.log(`[SHEET_UTIL INFO] TAB: Main application data sheet "${APP_TRACKER_SHEET_TAB_NAME}" not found in "${ss.getName()}". Creating...`);
      try {
        appTrackerSheet = ss.insertSheet(APP_TRACKER_SHEET_TAB_NAME); // Insert at default position, can be reordered later if needed
        // ss.setActiveSheet(appTrackerSheet); // Not always necessary to activate here

        // Attempt to delete the default "Sheet1" if it exists and is not one of our key sheets
        ...
        if (defaultSheet && defaultSheet.getSheetId() !== appTrackerSheet.getSheetId()) {
            ...
            for (const name of keySheetNames) {
                ...
                if (sheet && sheet.getSheetId() === defaultSheet.getSheetId()) {
                    isKeySheet = true;
                    break;
                }
            }
            if (!isKeySheet && ss.getSheets().length > 1) { // Only delete if it's not a key sheet and not the only sheet
                 try { ss.deleteSheet(defaultSheet); Logger.log(`[SHEET_UTIL INFO] TAB: Removed default 'Sheet1' as it was not a recognized key tab.`); }
        ...
        setupSheetFormatting(appTrackerSheet);
        ...
        return { spreadsheet: ss, sheet: null }; // Return spreadsheet object even if sheet creation failed
      }
    } else {
      Logger.log(`[SHEET_UTIL INFO] TAB: Found existing main application data sheet "${APP_TRACKER_SHEET_TAB_NAME}".`);
      // Ensure formatting is applied/verified for existing "Applications" sheet
      setupSheetFormatting(appTrackerSheet);
    }
  } else {
    // This case should have been caught earlier if 'ss' couldn't be obtained.
    Logger.log(`[SHEET_UTIL FATAL] SPREADSHEET: Spreadsheet object ('ss') is null. Cannot proceed to get/create "${APP_TRACKER_SHEET_TAB_NAME}".`);
    return { spreadsheet: null, sheet: null };
  }
