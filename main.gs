...
function initialSetup_LabelsAndSheet() {
  ...
  let trackerToProcessLabelId = null; // Declared
  const trackerToProcessLabelNameConst = TRACKER_GMAIL_LABEL_TO_PROCESS; 

  try {
    // Ensure all parent and sibling labels are created
    getOrCreateLabel(MASTER_GMAIL_LABEL_PARENT); Utilities.sleep(200);
    getOrCreateLabel(TRACKER_GMAIL_LABEL_PARENT); Utilities.sleep(200);
    const toProcessLabelObject = getOrCreateLabel(trackerToProcessLabelNameConst); // <<<< Get the object
    Utilities.sleep(100); 
    getOrCreateLabel(TRACKER_GMAIL_LABEL_PROCESSED); Utilities.sleep(100);
    getOrCreateLabel(TRACKER_GMAIL_LABEL_MANUAL_REVIEW); Utilities.sleep(100);
    Logger.log("[INFO] SETUP (Tracker): Standard getOrCreateLabel calls completed for tracker labels.");
    
    // --- Log inspection of toProcessLabelObject ---
    ...
    try {
        ...
        if (labelsListResponse.labels && labelsListResponse.labels.length > 0) {
            ...
            if (targetLabelInfo && targetLabelInfo.id) {
                trackerToProcessLabelId = targetLabelInfo.id; // Assigned here
  ...
  // This part of the summary message generation relies on trackerLabelsOk
  if (trackerLabelsOk) {
    ...
    overallSuccessThisModule = false;
  }
  ...
  if (trackerToProcessLabelId) { 
    ...
    let filterCreationAttempted = false; // Flag to see if we even try
    try {
        const gmailApiServiceForFilter = Gmail; 
        let filterExists = false;
        // Ensure advanced service is good
        ...
        const existingFiltersList = existingFiltersResponse.filter;

        if (existingFiltersList && existingFiltersList.length > 0) {
            for (const filter of existingFiltersList) {
                if (filter.criteria && filter.criteria.query === TRACKER_GMAIL_FILTER_QUERY_APP_UPDATES && 
                    filter.action && filter.action.addLabelIds && filter.action.addLabelIds.includes(trackerToProcessLabelId)) {
                    filterExists = true;
                    break;
                }
            }
        ...
        if (!filterExists) {
            filterCreationAttempted = true; // We will attempt to create it
            const filterResource = {
                criteria: { query: TRACKER_GMAIL_FILTER_QUERY_APP_UPDATES }, 
                action: { addLabelIds: [trackerToProcessLabelId], removeLabelIds: ['INBOX'] } 
            };
            Logger.log(`[DEBUG] SETUP (Tracker): Attempting to create filter with resource: ${JSON.stringify(filterResource)}`);
            const createdFilterResponse = gmailApiServiceForFilter.Users.Settings.Filters.create(filterResource, 'me');
            // Check response from create call
            if (createdFilterResponse && createdFilterResponse.id) {
                ...
                // This case might happen if create doesn't throw but doesn't return expected ID.
        ...
        if (e.message && e.message.toLowerCase().includes("filter already exists")) {
            ...
            messages.push(`Tracker App Update Filter: FAILED CRITICALLY - ${e.message}.`);
            overallSuccessThisModule = false;
        }
    }
  } else { 
      ...
      // overallSuccessThisModule = false; // If filter is critical
  }
// --- END NEW FILTER CREATION ---
  ...
  if (!ss || !dataSh) {
    ...
    if (dataSh && dataSh.getLastRow() <= 1) { // Ensure dataSh is valid
      ...
      try {
        ...
        // Ensure dummy data rows match the expected number of columns for the "Applications" sheet
        dummyRowsData = dummyRowsData.map(row => {
            while (row.length < TOTAL_COLUMNS_IN_APP_SHEET) row.push(""); // Use TOTAL_COLUMNS_IN_APP_SHEET from Config.gs
            return row.slice(0, TOTAL_COLUMNS_IN_APP_SHEET);             // Use TOTAL_COLUMNS_IN_APP_SHEET from Config.gs
        });

        // Write dummy data, ensuring the range matches the number of columns
        dataSh.getRange(2, 1, dummyRowsData.length, TOTAL_COLUMNS_IN_APP_SHEET).setValues(dummyRowsData); // Use TOTAL_COLUMNS_IN_APP_SHEET
        dummyDataWasAdded = true;
        Logger.log(`[INFO] SETUP: Added ${dummyRowsData.length} dummy rows to "Applications" sheet.`);
      } catch (e) {
        Logger.log(`[ERROR] SETUP: Failed to add dummy data to "Applications" sheet: ${e.toString()} \nStack: ${e.stack}`);
        // overallSuccessThisModule = false; // If you have a local flag
      }
    }
